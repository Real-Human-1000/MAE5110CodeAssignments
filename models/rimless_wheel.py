...
def dynamics(t, state, params, poincare_section=None):
    ...
    #   state: 2x1 array representing state. This may be modified in-place if there is a collision
    #   params: dict of params as generated by generate_params()
    #   poincare_section: if an empty list is provided here, states just after collision will be appended to it
    # Outputs: 2xn array representing the derivative of the state
    ...
    if rel_angle > half_spoke_angle:
        # Transition to the next spoke
        # The "next spoke" has probably actually penetrated into the ground by a tiny bit now
        # Let's find the state that the pendulum would have been at when it reached the contact angle exactly
        ...
        # At the contact state, the pendulum must have had a relative angle equal to half_spoke_angle
        contact_potential_energy = params["gravity"] * params["mass"] * np.cos(half_spoke_angle + slope_angle) * params["spoke_length"]
        # Solve for the contact kinetic energy
        contact_kinetic_energy = this_total_energy - contact_potential_energy
        contact_ang_vel = np.sqrt(contact_kinetic_energy / (0.5 * params["mass"])) / params["spoke_length"]
        # Projecting the previous velocity onto the arc of the new spoke to preserve the tangent component:
        angular_velocity = contact_ang_vel * np.cos(2 * half_spoke_angle)  #*= np.cos(2 * half_spoke_angle)
    ...
    if rel_angle < -half_spoke_angle:
        # The relative angle is behind the back spoke
        # Somehow, the wheel has rolled backwards
...
def generate_params():
    # Generate some normal params for this system
    # Params generally include things that define the system and don't change
    # Inputs: None
    # Outputs: dict of params
